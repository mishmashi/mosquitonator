...
def load_data():
        ...
        questions = [col for col in df.columns if col not in ("Species", "Probability", "Region", "Image")]

        database = []
        for _, row in df.iterrows():
            entry = {"name": row["Species"], "prob": row["Probability"], "image": row["Image"], "region": row["Region"]}
