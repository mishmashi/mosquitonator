...
compare = st.selectbox(
    "Check if the specimen is a specific species:",
    name_list,
    index=None,
...
def load_gt():
    df = pd.read_csv("Compact descriptions.csv", header=0)
    database = []
    for _, row in df.iterrows():
        entry = {"name": row["Species"], "gt": row["V_0"]}
        database.append(entry)
    return database
...
st.title(f"Anopheles quick check: {compare}")
